...
static volatile int8_t Channel[_Nbr_10timers ];             // counter for the servo being pulsed for each timer (or -1 if refresh interval)
...
// convenience macros
#define SERVO_INDEX_TO_TIMER(_servo_nbr) ((timer10_Sequence_t)(_servo_nbr / SERVOS_PER_TIMER)) // returns the timer controlling this servo
...
static inline void handle_interrupts(timer10_Sequence_t timer, volatile uint16_t *TCNTn, volatile uint16_t* OCRnA)
{
  if( Channel[timer] < 0 )
...
static void initISR(timer10_Sequence_t timer)
{
#if defined (_useTimer1)
  if(timer == _timer1) {
...
static void finISR(timer10_Sequence_t timer)
{
...
static boolean isTimerActive(timer10_Sequence_t timer)
{
...
uint8_t OdrinServo::attach(int pin, int min, int max)
{
  if(this->servoIndex < MAX_SERVOS ) {
    ...
    timer10_Sequence_t timer = SERVO_INDEX_TO_TIMER(servoIndex);
