...
uint8_t HexaServo::attach(int pin)
{
  return this->attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);
}

uint8_t HexaServo::attach(int pin, int min, int max)
{
...
void HexaServo::write(int value)
{
...
void HexaServo::writeMicroseconds(int value)
{
...
*/
void HexaServo::write(int value, uint8_t speed) {
...
void HexaServo::write(int value, uint8_t speed, bool wait) {
  write(value, speed);
...
void HexaServo::slowmove(int value, uint8_t speed) {
  // legacy function to support original version of HexaServo
  write(value, speed);
}
...
int HexaServo::read() // return the value as degrees
{
  return  map( this->readMicroseconds()+1, SERVO_MIN(), SERVO_MAX(), 0, 180);
}

int HexaServo::readMicroseconds()
{
...
bool HexaServo::attached()
{
  return servos[this->servoIndex].Pin.isActive ;
}

uint8_t HexaServo::sequencePlay(servoSequencePoint sequenceIn[], uint8_t numPositions, bool loop, uint8_t startPos) {
...
uint8_t HexaServo::sequencePlay(servoSequencePoint sequenceIn[], uint8_t numPositions) {
  return sequencePlay(sequenceIn, numPositions, true, 0);
}

void HexaServo::sequenceStop() {
  write(read());
  this->curSeqPosition = CURRENT_SEQUENCE_STOP;
}
...
/*
  To do
int HexaServo::targetPosition() {
  byte channel = this->servoIndex;
  return map( servos[channel].target+1, SERVO_MIN(), SERVO_MAX(), 0, 180);
}

int HexaServo::targetPositionMicroseconds() {
  byte channel = this->servoIndex;
